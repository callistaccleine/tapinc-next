...
export async function POST(req: Request) {
  ...
  const signerCertPath = process.env.PASSKIT_SIGNER_CERT_PATH || process.env.PASSKIT_CERT_PEM;
  const signerKeyPath = process.env.PASSKIT_SIGNER_KEY_PATH || process.env.PASSKIT_KEY_PEM;
  const signerKeyPassphrase =
    process.env.PASSKIT_SIGNER_KEY_PASSWORD || process.env.PASSKIT_CERT_PASSWORD;

  const hasP12Config = Boolean(certPath && certPassword);
  const hasPemConfig = Boolean(signerCertPath && signerKeyPath);

  if (!wwdrPath || !teamIdentifier || !passTypeIdentifier || !orgName || (!hasP12Config && !hasPemConfig)) {
  ...
  try {
    ...
    const wwdrSource = await readCertificateSource(wwdrPath, "WWDR certificate");
    ...
    if (hasP12Config && certPath && certPassword) {
      const p12Source = await readCertificateSource(certPath, "PKCS#12 certificate");
      const extracted = extractP12Certificates(p12Source, certPassword, "PKCS#12 certificate");
      ...
      const signerCertSource = await readCertificateSource(signerCertPath, "Signer certificate");
      const signerKeySource = await readCertificateSource(signerKeyPath, "Signer key");
      signerCert = normalizePemCertificate(signerCertSource, "Signer certificate");
    ...
    const pass = await PKPass.from(
      ...
      },
      {
        ...
        generic: {
          ...
          auxiliaryFields: [{ key: "company", label: "COMPANY", value: company }],
        },
      }
    );
    ...
    pass.auxiliaryFields.push({ key: "company", label: "COMPANY", value: company });
    pass.setBarcodes({
      message: barcodeMessage,
      format: "PKBarcodeFormatQR",
      messageEncoding: "iso-8859-1",
